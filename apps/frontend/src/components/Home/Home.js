  ...
  return (
    <Box sx={{ width: '100%', maxWidth: 1180, mx: 'auto', p: { xs: 2, md: 4 } }}>
      ...
      <Grid container spacing={2.5}>
        <Grid item xs={6} md={3}>
          <KpiCard icon={<CalendarMonthIcon />} label="Séances" value={dashboard.nbr_of_seances} helper="ce mois-ci" />
        </Grid>
        <Grid item xs={6} md={3}>
          <KpiCard icon={<EmojiEventsOutlinedIcon />} label="Niveau max" value={formatDifficulty(dashboard.max_lvl)} helper="voie terminée" />
        </Grid>
        <Grid item xs={6} md={3}>
          <KpiCard icon={<ExploreOutlinedIcon />} label="Mur exploré" value={`${Math.round(dashboard.coverage * 100)}%`} helper="sur le plan actuel" />
        </Grid>
        <Grid item xs={6} md={3}>
          <KpiCard icon={<VerticalAlignTopIcon />} label="En tête" value={`${Math.round(dashboard.tete_ratio * 100)}%`} helper="de vos essais" />
        </Grid>

...
export default function WallAnalysis() {
  ...
  useEffect(() => {
    if (!versionId) {
      navigate('/listevoies', { replace: true });
      return undefined;
    }
    let live = true;
    setLoading(true);
    ...
    return () => { live = false; };
  }, [navigate, setErrorMessage, versionId]);
  ...
  const changeVersion = (event) => {
    ...
    if (nextVersion && nextVersion !== versionId) navigate(`/analyse-mur?version=${nextVersion}`);
  };
  ...
  return (
    <Box sx={{ width: '100%', maxWidth: 1240, mx: 'auto', p: { xs: 2, md: 4 } }}>
      <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" spacing={2} sx={{ mb: 3.5 }}>
        <Box>
          <Button startIcon={<ArrowBackIcon />} onClick={() => navigate('/listevoies')} sx={{ mb: 1, ml: -1, color: 'text.secondary' }}>Retour aux voies</Button>
      ...
      {loading ? (
        ...
        <>
          <Grid container spacing={2.25}>
            <Grid size={{ xs: 6, md: 3 }}><StatCard icon={<RouteOutlinedIcon />} value={analysis.total_routes} label="Voies équipées" helper={`${analysis.equipped_lanes} couloirs occupés`} /></Grid>
            <Grid size={{ xs: 6, md: 3 }}><StatCard icon={<TrendingUpIcon />} value={formatDifficulty(analysis.average_difficulty)} label="Niveau moyen" helper={`de ${formatDifficulty(analysis.lowest_difficulty)} à ${formatDifficulty(analysis.highest_difficulty)}`} tint="#e28d39" /></Grid>
            <Grid size={{ xs: 6, md: 3 }}><StatCard icon={<EmojiEventsOutlinedIcon />} value={analysis.climbs} label="Passages enregistrés" helper={`${analysis.unique_climbed_routes} voies déjà grimpées`} tint="#6787b7" /></Grid>
            <Grid size={{ xs: 6, md: 3 }}><StatCard icon={<CheckCircleOutlineIcon />} value={`${successRate}%`} label="Taux de réussite" helper={`${analysis.tops} réussites au total`} tint="#9b5b7c" /></Grid>
          </Grid>
